...
GRANT_SQL: list[str] = [
    ...
    f"GRANT SELECT, INSERT, UPDATE, DELETE ON ALL TABLES IN SCHEMA {OPS_SCHEMA} TO {APP_ROLE}",
...
def upgrade() -> None:
    bind = op.get_bind()
    # Tables are created from ORM metadata so the migration cannot drift from the
    # models the repositories use. Schemas are pre-created by db-create bootstrap.
